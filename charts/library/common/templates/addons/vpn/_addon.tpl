...
{{- if .Values.addons.vpn.enabled -}}
  ...
  {{- if and .Values.addons.vpn.configFile (not .Values.addons.vpn.configFileSecret) }}
    {{- $secret := include "ntrp.common.addon.vpn.secret" . -}}
    {{- if $secret -}}
  ...
  {{- $scriptVolume := include "ntrp.common.addon.vpn.scriptsVolumeSpec" . | fromYaml -}}
  {{- if $scriptVolume -}}
  ...
  {{- $configVolume := include "ntrp.common.addon.vpn.configVolumeSpec" . | fromYaml }}
  {{ if $configVolume -}}
  ...
  {{- $networkpolicy := include "ntrp.common.addon.vpn.networkpolicy" . -}}
  {{- if $networkpolicy -}}
