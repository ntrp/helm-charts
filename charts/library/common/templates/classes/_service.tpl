...
*/}}
{{- define "ntrp.common.class.service" -}}
...
{{- $serviceName := include "ntrp.common.lib.chart.names.fullname" . -}}
{{- if and (hasKey $values "nameOverride") $values.nameOverride -}}
  {{- $serviceName = printf "%v-%v" $serviceName $values.nameOverride -}}
{{ end -}}
{{- $svcType := $values.type | default "" -}}
{{- $primaryPort := get $values.ports (include "ntrp.common.lib.service.primaryPort" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  {{- end }}
  {{- end }}
  {{- with (merge ($values.extraSelectorLabels | default dict) (include "ntrp.common.lib.metadata.selectorLabels" . | fromYaml)) }}
